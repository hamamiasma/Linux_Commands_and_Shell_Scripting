...
# تمرين 3 - إدارة الملفات والمجلدات

## 3.1. البحث عن الملفات وتحديد موقعها

### الأمر: `find`

يُستخدم الأمر `find` للبحث عن الملفات داخل دليل معين. يمكنك البحث باستخدام عدة خصائص مثل الاسم، النوع، المالك، الحجم، أو التاريخ.

يقوم الأمر `find` بالبحث داخل شجرة المجلدات الكاملة انطلاقًا من الدليل المحدد.

**مثال:** للعثور على جميع الملفات المنتهية بـ `.txt` داخل دليل `/etc` وكل المجلدات الفرعية:

```bash
find /etc -name '*.txt'
```

> **ملاحظة:** قد تظهر رسائل "Permission denied" أثناء التنفيذ. هذا أمر طبيعي بسبب قيود الأذونات.
...
يُستخدم الأمر `rm` لحذف الملفات. من الأفضل استخدام الخيار `-i` لتأكيد الحذف قبل تنفيذ كل عملية.

**لحذف ملف باسم `myfile.txt`:**

```bash
rm -i myfile.txt
```

اضغط `y` للتأكيد أو `n` للإلغاء.
...
> **نصيحة:** عند حذف عدة ملفات باستخدام الرموز النمطية (wildcards)، يوصى باستخدام `-i` للحماية من الحذف غير المقصود. لا توجد طريقة لاسترجاع الملفات المحذوفة، لذا تأكد من أخذ نسخة احتياطية من ملفاتك المهمة.

---

## 3.3. نقل وإعادة تسمية ملف

### الأمر: `mv`

يمكنك استخدام الأمر `mv` لنقل الملفات أو إعادة تسميتها.
...
**للتحقق من إعادة التسمية:**

```bash
ls
```

**لنقل الملف إلى مجلد `/tmp`:**
...
**لنسخ الملف `user-info.txt` من `/tmp` إلى المجلد الحالي:**
...
🛡️ **تذكير:** كن دائمًا حذرًا عند الحذف أو النقل أو الكتابة فوق الملفات، ولا تنسَ عمل نسخ احتياطية دورية لملفاتك المهمة.
